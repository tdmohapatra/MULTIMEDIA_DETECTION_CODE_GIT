  ...
  ### YOLOv8 model placement and runtime switch
  - Put YOLO ONNX files under: `TDM_MULTIMEDIA DOTNET CORE/ImgToText/Uploads/models/`
  - Recommended filename: `yolov8n.onnx` (optional: `yolov8s.onnx`)
  - Do **not** place runtime models in `ImgToText/Models/` for inference loading.
  - In UI:
    - `DetectionClient/SceneAnalysis` -> select **Scene model backend = YOLOv8 ONNX**
    - `DetectionClient/ModelCanvas` -> select **Scene model backend = YOLOv8 ONNX**
  - Loader behavior:
    - If YOLO ONNX exists and backend is set to `yolo`, scene pipeline uses YOLO.
    - If not available, pipeline safely falls back to existing SSD/cascade paths.
  ...
  - `DetectorDiagnosticsReport` includes `inferenceProvider`, `sceneModelBackend`, per-module FPS/latency, and recent intelligence events
  - Scene UIs show:
    - model status dots,
    - SLO/latency status,
    - stage timing split,
